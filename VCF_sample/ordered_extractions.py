...
def return_seqs_v1(seq,size= 10,L= 1000,keep= ['A','T','G','C'],threshold= 0.01):
    '''
    ...
    seqL= len(seq)
    print('ref_seq length: {}'.format(seqL))
    ...
    while d < size:
        ...
        scrag= len(scrag)
        scrag_dump.append(scrag)
        #print(list(set(scrag)))
        #print(len(scrag))
        imp= scrag / L
        if imp < threshold:
            seq_dict[str(pos)]= {
    ...
    if len(scrag_dump):
        print('average N in fasta: {}. seqL= {}'.format(np.mean(scrag_dump),L))
    return seq_dict
