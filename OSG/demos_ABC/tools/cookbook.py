...
def write_args(args_dict,SIMname,SIM_dir,batch= 'NA'):
    '''
...
def cook_constants_rateVar(fasta_dict, mu= 1e-8, mut_file= 'mut_matrix_v0.txt', s1= 1092, NeC= 2e5, Nef= 4e5, Grate= 1.03,
    ...
    constants:
        ...
        - Nef: effective population size after change.
        - Grate: growth rate during change. 
        - mut_file: file of mutation rate by kmer.
    '''
    cookID= 'rateVar'
    sim_store= {}
    ...
    for chrom in fasta_dict.keys():
        for start in fasta_dict[chrom].keys():
            ...
            ### set up names and directories.
                ...
                ### write arguments to file
                write_args(sim_store[SIMname],SIMname,SIM_dir,batch= batch_name)            
                ### population identifiers file
                sample_sizes= [sim_store[SIMname][x] for x in ["s1"]]
                write_popIDs(sample_sizes,file_dir= SIM_dir)
...
def cook_constants_ABC(fasta_dict, demo_file= 'Prado&Martinez2013_M4A.txt', template= 'template_simple.slim', Nsamp= 5,anc_r= '0', sizes= 500, burnin= 5e4,
            mu= 1e-8, rec= 1e-8, bases= 'ACGT', rate_change= 10, rate_range= [1,2], Nmat= 1, sim_scale= 1,
            dir_data= "./data/", dir_vcf= "vcf_data/sims/",
            slim_dir= './', batch_name= ''):
    ...
    mutations_full_dict, mutations_full_list, mut_org= mutation_dict_full(bases= bases)
    var_store= {
        "M{}".format(x): rate_mods(mut_org,rate_range= rate_range,rate_change= rate_change, bases= bases,mu= mu / sim_scale) for x in range(1,Nmat+1)
    }
...
def cook_constants_ABC_ratevar(fasta_dict, demo_file= 'Prado&Martinez2013_M4A.txt', template= 'template_simple.slim', mut_file= 'M4AM0_grid.txt',Nsamp= 5,anc_r= '0', sizes= 500, burnin= 5e4,
            mu= 1e-8, rec= 1e-8, bases= 'ACGT', rate_change= 10, rate_range= [1,2], Nmat= 1, sim_scale= 1,
            dir_data= "./data/", dir_vcf= "vcf_data/sims/",
            slim_dir= './', batch_name= ''):
    '''
    ...
    for chrom in fasta_dict.keys():
        for start in fasta_dict[chrom].keys():
            ...
            for idx in range(Nsamp):
                ...
                ### set up names and directories.
                SIMname= batch_name + 'C{}.{}.{}'.format(chrom,str(start),idx)
                SIM_dir= dir_data + SIMname
